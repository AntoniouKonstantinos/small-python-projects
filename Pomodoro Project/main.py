...
def start_timer():
    ...
    reps += 1
    if reps % 8 == 0:
        title_label.config(text="Brake", fg=RED)
        count_down(LONG_BREAK_MIN * 60)
    elif  reps % 2 == 0:
        title_label.config(text="Brake", fg=PINK)
...
left_button = Button(text="Start", highlightthickness=0, command=start_timer)
left_button.grid(column=0, row=2)

right_button = Button(text="Reset", highlightthickness=0, command=reset_timer)
